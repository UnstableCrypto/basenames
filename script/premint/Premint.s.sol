...
///
/// @notice addr(PREMINT_PRIVATE_KEY) must be an approved `controller` on the UnstableRegistrar
contract Premint is Script {
    ...
    function run(string memory name, uint256 duration) external {
        ...
        if (!UnstableRegistrar(BASE_REGISTRAR).isAvailable(id)) {
            console.log("Name already registered");
            return;
        }

        // Premint name
        UnstableRegistrar(BASE_REGISTRAR).registerOnly(id, BASE_ECOSYSTEM_MULTISIG, duration);
    ...
    function verify(uint256 lines, string calldata file) public view {
        for (uint256 i; i < lines; i++) {
            ...
            bytes32 label = keccak256(bytes(name));
            uint256 id = uint256(label);
            if (UnstableRegistrar(BASE_REGISTRAR).nameExpires(id) == 0) {
                console.log("Not minted: ", name);
            } else if (UnstableRegistrar(BASE_REGISTRAR).ownerOf(id) != BASE_ECOSYSTEM_MULTISIG) {
